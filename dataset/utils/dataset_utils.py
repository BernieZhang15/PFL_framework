...
batch_size = 32
train_size = 0.4 # merge original training set and test set, then split it manually.
...
def separate_data(data, num_clients, num_classes, niid=False, balance=False, partition=None, class_per_client=20):
    ...
    if partition == 'pat':
        ...
        for i in range(num_classes):
            ...
            client_numbers = list(range(num_clients))
            ...
            for client, num_sample in zip(selected_clients, num_samples):
                if client not in data_idx_map.keys():
                    data_idx_map[client] = idx_for_each_class[i][idx:idx+num_sample]
                else:
                    data_idx_map[client] = np.append(data_idx_map[client], idx_for_each_class[i][idx:idx+num_sample], axis=0)
                idx += num_sample
