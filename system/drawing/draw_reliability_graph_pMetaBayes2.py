...
model_path = os.path.join('models', dataset, algorithm + "_server_5173_0.005" + ".pt")
...
for c in range(c_start, c_end):
    ...
    y_probs.append(stats[1])
    y_trues.append(stats[2])

y_probs = torch.cat(y_probs, axis=0)
y_trues = torch.cat(y_trues, axis=0)

test_ece = multiclass_calibration_error(y_probs, y_trues, num_classes=10, n_bins=15, norm="l1")
test_mce = multiclass_calibration_error(y_probs, y_trues, num_classes=10, n_bins=15, norm="max")

make_model_diagrams(y_probs, y_trues, test_ece, test_mce)

print(test_ece, test_mce)
